...
In this document, we detail the correct usage and derivation of this retro-reflective BRDF model within the microfacet framework~\cite{WalterMicrofacet,HeitzMicrofacet}.
We first recall some basic principles from microfacet theory (Section~\ref{sec:microfacets}), then show how this model can be steered from forward- to retro-reflection (Section~\ref{sec:backvector}), and further provide proof that the resulting BRDF is fully plausible (Section~\ref{sec:plausibility}).
...
Instead of half-vector $\vech$, we can use the back-vector $\vecb$~\cite{BelcourBackvector} defined as
\begin{equation}
...
\begin{figure}[h]
  ...
  \end{center}
  \caption{Geometry of the back vector, illustrated in two dimensions.}
...
There is experimental evidence~\cite{BelcourBackvector} that using a Fresnel term based on the back-vector can provide a good match to actual retro-reflective material measurements.
...
With Smith masking, this is actually identical to a regular microfacet BRDF, as $\vecvv$ and $\vecv$ share the same geometric configuration.
...
\begin{equation}\label{eq:nh}
  \begin{split}
    ...
    \end{split}
\end{equation}
As $D$ is reflection-symmetric around the normal, i.e. it takes the form $D(\vech) = D\left(\vdot{\vecn}{\vech}, \lvert \vdot{\mathbf{t_1}}{\vech} \rvert, \lvert \vdot{\mathbf{t_2}}{\vech} \rvert\right)$, we can thus infer that using it with the back-vector obeys symmetry.
...
\end{equation}    
Further, using equation \eqref{eq:kh} along with equations \eqref{eq:nh} and \eqref{eq:nk} we get
\begin{equation}
  \begin{split}
  G_1(\vecvv, \vecl) &=
