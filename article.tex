...
%    \begin{tabular} {|l|l|}
      ...
      \hline
      $\mathbf{t_1}, \mathbf{t_2}$ & tangent and bitangent (forming an orthonormal coordinate frame with the macronormal) \\
      \hline
...
Similarly, using the half-vector of refraction $\vecht$ as selector yields the corresponding BTDF:
\begin{equation}
  f_t(\vecv, \vecl) =
...
In this work, we show that using the \emph{back vector}~\cite{BelcourBackvector} instead as a microfacet selector produces a highly plausible model of retro-reflection. Before detailing the impact of such a change, we review some standard properties of microfacet models.
...
\begin{equation}
G_{2}(\vecv, \vecl, \vech) = G_1(\vecv, \vech) \times G_1(\vecl, \vech)\ .
\end{equation}
...
\paragraph*{Shadowing \& Masking.}
We assume Smith or V-cavity masking is applied to the mirrored microfacet distribution.
With Smith masking, this is actually identical to a regular microfacet BSDF, as $\vecvv$ and $\vecv$ share the same geometric configuration. For V-cavity masking, we take $\vecvv$ as view direction and, consequently, $\vecb$ as microfacet normal.
...
\begin{figure}[tbh]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/brdf_plots.pdf}
...
We briefly prove here that the retro-reflective microfacet model of Equation~\eqref{eq:retro_microfacet_model} is plausible, i.e. that it fulfills symmetry and energy conservation requirements.
...
\begin{equation}
  ...
  f_r(\vecvv, \vecl) &=
  \frac{D(\vech(\vecvv, \vecl)) \, G_{2}(\vecvv, \vecl) \, F\left(\vecvv, \vech(\vecvv, \vecl)\right)}{4 \vdot{\vecvv}{\vecn} \vdot{\vecl}{\vecn} }\\
  &=
  \frac{D(\vech(\vecll, \vecv)) \, G_{2}(\vecll, \vecv) \, F\left(\vecll, \vech(\vecll,\vecv)\right)}{4 \vdot{\vecv}{\vecn} \vdot{\vecll}{\vecn} }\\
  &=
  f_r(\vecll, \vecv) =
